...
Original file is located at
    https://colab.research.google.com/drive/1i4KEWR5NleS3kEd8jbeaKUES_1SlFEfj

# Bank Customer Churn Prediction

First step: problem framing, knowing what problem we are trying to solve--identify customers who are likely to close their account in the future, and predict the probability that a customer is about to churn.
---
Motivation: Take actions to keep the customers that are about to churn. For example, send discount, make new products and send emails, push营销信息；返利、设置打卡积分系统；分析app本身，是否因为转化路径太长导致流失、是否流失在前期用户体验上、是否流失在产品体验交互设计上；分析用户行为，根据不同群体制定不同运营策略：年轻群体、高收入人群、理财达人等。
...
"""Tenure: 使用产品时长； IsActiveMember：  how to tell?"""
...
"""RowNumber == #CustomerId indicates that there's no duplicates; These two are irrelavant variables; Surname(last name) doesn't have an impact on bank behavior in common sense; 3 countries; Balance: over 3k people have same balance(0?)"""
...
"""Ratio of Germany user that churns is the highest（might related to culture)； Female tends to churn; Non-active users tend to churn
...
#train: test = 3:1 # use stratified sampling to balance the ratio of people churn and stay (especially when data size is small)
X_train, X_test, y_train, y_test = model_selection.train_test_split(X, y, test_size=0.25, stratify = y, random_state=888) 
...
2. training: 7500 '0' (can not build model on only churn people)
---
...
X_test = OneHotEncoding(X_test, enc_ohe, categories)#apply enc_ohe to testing set

X_train.head()

# Ordinal encoding (one-hot is better, since ordinal gives a distance of categorical variables)
...
X_test = OneHotEncoding(X_test, enc_ohe1, categories1)#apply enc_ohe to testing set
...
#Loss/cost function --> (wx + b - y) ^2 + ƛ * |w| --> lambda is a hyperparameter
...
"""improve performance: use more complicated models, get rid of some irrelavant features, get more useful features, etc
...
#The higher, the more important the feature. The importance of a feature is computed as the (normalized) total reduction of the criterion brought 
